export const buildValidators = ({
  ...
  validators = validators.map(validator => {
    ...
    if (!Array.isArray(validator))
      throw new Error(
        `The \`${JSON.stringify(validator)}\` validator is invalid!`
      );
    ...
    if (typeof validation === "string" && !message)
      message = messages[validation];
    if (typeof validation === "string") validation = validations[validation];
    if (typeof validation !== "function")
      throw new Error(
        `The validation \`${JSON.stringify(validation)}\` must be a function!`
      );
    if (message && typeof message !== "string")
      throw new Error(
        `The message \`${JSON.stringify(validation)}\` must be of type string!`
      );

    return [
      v => validation(v, { values, ...args }),
      translate(message, { label, ...args })
    ];
  });
